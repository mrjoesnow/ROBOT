...
void throwRoutine(int newPowerLevel) {
  ...
  // Send power level to slave
  Wire.beginTransmission(I2C_DEV_ADDR);
  Wire.write(0x01); // Register for power level
  Wire.write(newPowerLevel / 500); // Scale power level to fit in a byte
  Wire.endTransmission();
  ...
  // Send throw signal to slave
  sendThrowSignalToSlave();
}
